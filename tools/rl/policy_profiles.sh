...
rl_policy_load_properties() {
  local policy_id="$1"
  local default_profile="${rl_policy_config_root}/default/profile.properties"
  local default_rewards="${rl_policy_config_root}/default/reward.properties"
  ...
  if [[ -f "${default_rewards}" ]]; then
    set -a
    # shellcheck source=/dev/null
    source "${default_rewards}"
    set +a
  fi
  ...
  if [[ "${policy_id}" != "default" && -f "${policy_rewards}" ]]; then
    set -a
    # shellcheck source=/dev/null
    source "${policy_rewards}"
    set +a
  fi
  if [[ "${policy_id}" != "default" && -f "${policy_profile}" ]]; then
    set -a
    # shellcheck source=/dev/null
...
rl_policy_run_batch() {
  ...
  for policy_id in "${policy_ids[@]}"; do
    ...
    (
      ...
      export RL_BEST_OUTPUT="${rl_policy_asset_root}/${policy_id}/${rl_policy_file_name}"
