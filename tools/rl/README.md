...
- `rl-checkpoints-navigation-route-awareness/` or `rl-checkpoints-route-awareness/`
...
cars. Its exported format is `ratass-rl-policy-v3`, tied to the route-awareness
direct safe-circle objective. The default checkpoint directory is
`rl-checkpoints-route-awareness`; do not resume older checkpoints from
`rl-checkpoints`, `rl-checkpoints-tactical`, `rl-checkpoints-circle`,
`rl-checkpoints-navigation`, `rl-checkpoints-direct-circle`, or
`rl-checkpoints-direct-circle-route`, because those were trained with previous
observation, action, or reward meanings.
...
python tools/rl/train_rllib.py \
  ...
  --checkpoint-dir rl-checkpoints-navigation-route-awareness \
  --controlled-agents 2 \
  --field-size 2 \
  --iterations 100
```
...
The combat helper resumes `rl-checkpoints-route-awareness`. The navigation
wrapper resumes `rl-checkpoints-navigation-route-awareness`. Both train in repeated chunks,
...
- Observation size: `45` floats per learner.
- Action size: `2` floats per learner, `[throttle, turn]`, each in `[-1, 1]`.
- Observation floats `30..36` are the map-routed drive target: relative x,
  relative y, car-local forward alignment, car-local side alignment, normalized
  target distance, velocity toward the target, and a final-target flag. The
  routed target can point to an intermediate waypoint when a direct line to the
  circle crosses a hole.
- Observation floats `37..38` are danger-motion awareness: velocity along the
  recovery direction near hazards, and unsafe velocity into hazards.
- Observation floats `39..44` are local hazard clearances for forward,
