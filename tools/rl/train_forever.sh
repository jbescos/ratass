...
run_curriculum_phase() {
  ...
  local phase_train_batch_size="${RL_TRAIN_BATCH_SIZE:-4096}"
  ...
  if [[ "${phase_route_targets}" == "-2" ]]; then
    ...
    phase_map_ids="${RL_LAP_REAL_MAP_IDS:-auto-game}"
    phase_best_eval_map_ids="${RL_LAP_REAL_BEST_EVAL_MAP_IDS:-${phase_map_ids}}"
  elif [[ "${phase_route_targets}" == "-6" ]]; then
  ...
  if [[ "${iterations}" -le 0 ]]; then
    echo "curriculum_phase_skip policy=${RL_POLICY_ID:-legacy} profile=${RL_POLICY_INDEX:-?}/${RL_POLICY_TOTAL:-?} phase=${phase} iterations=${iterations} route_targets=${phase_route_targets} route_target_fraction=${phase_route_target_fraction} train_batch_size=${phase_train_batch_size} maps=${phase_map_ids:-all}"
    return
  fi
  if [[ ! "${phase_train_batch_size}" =~ ^[1-9][0-9]*$ ]]; then
    echo "invalid_train_batch_size=${phase_train_batch_size} phase=${phase} expected=positive_integer" >&2
    exit 2
  fi
  ...
  append_phase_state \
    ...
    "train_batch_size=${phase_train_batch_size}" \
...
normalize_route_stage() {
  ...
  case "${normalized_spec}" in
    ...
    "lap_real3"|"real_lap3"|"real-lap3"|"game_lap3"|"game-lap3"|"lap-real3") printf '%s\n' "-6" ;;
    "lap"|"full"|"-1") printf '%s\n' "-1" ;;
    "")
...
ray_node_ip="${RL_RAY_NODE_IP:-127.0.0.2}"
...
reward_drift="${RL_REWARD_DRIFT:-0.0}"
reward_route_alignment="${RL_REWARD_ROUTE_ALIGNMENT:-0.0}"
reward_steering_penalty="${RL_REWARD_STEERING_PENALTY:-0.010}"
reward_steering_change_penalty="${RL_REWARD_STEERING_CHANGE_PENALTY:-0.0}"
reward_pedal_change_penalty="${RL_REWARD_PEDAL_CHANGE_PENALTY:-0.0}"
...
reward_no_progress_penalty="${RL_REWARD_NO_PROGRESS_PENALTY:-500.0}"
reward_stationary_penalty="${RL_REWARD_STATIONARY_PENALTY:-0.50}"
reward_off_road_recovery="${RL_REWARD_OFF_ROAD_RECOVERY:-4.0}"
reward_off_road_failure_penalty="${RL_REWARD_OFF_ROAD_FAILURE_PENALTY:-500.0}"
...
common_args=(
  ...
  --reward-drift "${reward_drift}"
  --reward-route-alignment "${reward_route_alignment}"
  --reward-steering-penalty "${reward_steering_penalty}"
  --reward-steering-change-penalty "${reward_steering_change_penalty}"
  --reward-pedal-change-penalty "${reward_pedal_change_penalty}"
  ...
  --stationary-grace-action-steps "${stationary_grace_action_steps}"
  --reward-stationary-penalty "${reward_stationary_penalty}"
